...
void launch_fattn(
    ...
    // NOTE: upstream replaced the pool-allocated K_f16/V_f16 dequant scratch with pre-reserved
    // buffers obtained here (f16_extra.K / f16_extra.V), consumed by the downstream branches.
    // The turboquant HIP/ROCm pool-bypass OOM fix that used to live here is obsolete — there is
    // no more ggml_cuda_pool_alloc<half> for the f16 scratch. If quantized-KV OOM resurfaces on
    // HIP, re-apply the fix inside the get_f16_extra_data allocation path, not here. (Prod runs
    // CUDA on the RTX 3060, so the HIP path is not exercised in deployment.)
