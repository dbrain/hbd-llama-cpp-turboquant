...
ggml_tensor * llm_build_qwen35::build_layer_attn_linear(
    ...
    // The default get_rows promotes to F32 — fine even with F16 ssm_states_all. Tested
    // routing F16 throughout the gather (ggml_get_rows_typed), but the kernel read side
    // wasn't bandwidth-bound, and the half pack/unpack overhead net-regressed (lap-6).
    ...
    // Fused state writeback: pass the persistent state slot directly into the
    // GDN op so the new state is written there as a side effect, eliminating the
    // trailing ggml_cpy on the fused path. On the chunking/autoregressive
    // fallbacks the writeback is ignored and we emit the CPY explicitly.
    ...
    if (!use_fused_gdn) {
        // Fallback path (chunking / autoregressive): commit new state via CPY.
