...
llama_model_qwen35::graph_mtp::graph_mtp(const llama_model & model, const llm_graph_params & params)
    ...
    // hparams.n_layer() counts only the main model layers (excludes the n_layer_nextn
    // MTP layers); n_layer_all = n_layer() + n_layer_nextn. The MTP layer is stored
    // immediately after the main layers in model.layers[], i.e. at index n_layer().
