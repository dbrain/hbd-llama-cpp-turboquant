...
llama_kv_cache::llama_kv_cache(
    ...
    // upstream renamed the local `n_layer_kv` (= hparams.n_layer_kv(), now removed) to
    // `n_layer` (= hparams.n_layer_all); the per-layer loop below skips non-KV layers
    // via has_kv(il). Over-allocating tensor-overhead metadata for non-KV layers is benign.
    ...
    for (uint32_t il = 0; il < n_layer; il++) {
        ...
        ggml_type layer_type_v = type_v;
        {
            ...
            // `n_layer` (= hparams.n_layer_all) is already in scope from the ctor body above.
    ...
    // NOTE: turboquant keeps attn rotation OFF by default (set false above, opt-in via the
    // LLAMA_ATTN_ROT_{K,V}_OVERRIDE env gates). We do NOT restore upstream's unconditional
    // enablement. The DeepSeek-V3.2 DSA lightning indexer, however, *requires* the Hadamard
    // tensors regardless — keep that one upstream special-case (no-op for archs we run).
